...
def get_TP(target, prediction, threshold):
    ...
    Parameters:
    ----------------
    target: the ground truth , np.array
...
def get_recall(target, prediction, threshold):
    ...
    log('true positives={0}'.format(tp))
    log('false negatives={0}'.format(fn))
    if tp + fn <= 0.0:
...
def get_precision(target, prediction, threshold):
    ...
    log('true positives={0}'.format(tp))
    log('false positives={0}'.format(fp))
    if tp + fp <= 0.0:
...
def get_relative_error(target, prediction):
    ...
    return np.mean(np.nan_to_num(
        np.abs(target - prediction) / np.maximum(target, prediction)))
